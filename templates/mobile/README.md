...
Internal iOS builds (development, staging) need devices registered in your Expo account. To install the builds in iOS the devices need to be registered. Not required for Android.
```bash
...
- `EXPO_PUBLIC_ENVIRONMENT` — already set to `development`; can also prefix commands with `EXPO_PUBLIC_ENVIRONMENT=development|staging|production` if desired.
...
Expo GO will not work because there are native plugins added to the app already in the template. So a development client must be built and installed to run the app instead of Expo GO.
...
```bash
EXPO_PUBLIC_ENVIRONMENT=staging npm run start
...
When only typescript code changes (no native code or native plugin added), a new build is not required. Instead an update needs to be published and the apps just fetch the latest based on the update strategy configured in the app.
